import { Router } from 'express';
import authRoutes from './auth.routes.js';
import usersRoutes from './users.routes.js';
import machinesRoutes from './machines.routes.js';
import productsRoutes from './products.routes.js';
import cartRoutes from './cart.routes.js';
import feedbackRoutes from './feedback.routes.js';
import notificationsRoutes from './notifications.routes.js';
import paymentsRoutes from './payments.routes.js';
import campaignsRoutes from './campaigns.routes.js';
import contentRoutes from './content.routes.js';
import machinesQrRoutes from './machines.qr.routes.js';
import healthRoutes from './health.routes.js';
import docsRoutes from './docs.routes.js';
import legacyRoutes from './legacy.routes.js';
import adminApiRoutes from './admin.api.routes.js';
import adminCouponsRoutes from './admin-coupons.routes.js';
import couponsRoutes from './coupons.routes.js';
import adminVouchersRoutes from './admin-vouchers.routes.js';
import vouchersRoutes from './vouchers.routes.js';
import adminStepChallengesRoutes from './admin-step-challenges.routes.js';
import stepChallengesRoutes from './step-challenges.routes.js';
export const router = Router();
router.use('/auth', authRoutes);
router.use('/users', usersRoutes);
router.use('/machines', machinesRoutes);
router.use('/products', productsRoutes);
router.use('/cart', cartRoutes);
router.use('/feedback', feedbackRoutes);
router.use('/notifications', notificationsRoutes);
router.use('/payments', paymentsRoutes);
router.use('/campaigns', campaignsRoutes);
router.use('/coupons', couponsRoutes);
router.use('/vouchers', vouchersRoutes);
router.use('/content', contentRoutes);
router.use('/machines', machinesQrRoutes);
router.use('/health', healthRoutes);
router.use('/docs', docsRoutes);
router.use('/admin', adminApiRoutes);
router.use('/admin', adminCouponsRoutes);
router.use('/admin', adminVouchersRoutes);
router.use('/admin', adminStepChallengesRoutes);
router.use('/step-challenges', stepChallengesRoutes);
router.get('/', (_req, res) => res.render('welcome', { title: 'Vend-IT' }));
router.use('/', legacyRoutes);
export default router;
