...
DROP TABLE IF EXISTS campagin CASCADE;
...
CREATE TABLE IF NOT EXISTS user_loyality_points (
    id BIGSERIAL PRIMARY KEY,
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS machine_webhooks (
    id BIGSERIAL PRIMARY KEY,
    source VARCHAR(100),
    headers JSONB,
    payload JSONB,
...
CREATE INDEX IF NOT EXISTS idx_machines_location ON machines(location_latitude, location_longitude);
...
CREATE INDEX IF NOT EXISTS idx_campaign_views_user ON campaign_views(user_id);
CREATE INDEX IF NOT EXISTS idx_campaign_views_campaign ON campaign_views(campaign_id);
